...
export default function IncidentInformationStage({ formData, setFormData, failedStep, setValidStep }: StageProps) {
    return (
        <div className="min-w-0 flex-1 text-sm/6">
            ...
            <div className="mt-4">
                <label className="block text-sm/6 font-medium text-gray-900">When did this Incident occur?</label>
                <div className="mt-2">
                    <DateInput
            ...
            <div className="mt-4">
                <div>
                    <label className="block text-sm/6 font-medium text-gray-900">Incident Type</label>
                </div>
                <div className="mt-1 grid grid-cols-1">
                    <SelectInput
                        value={descriptors.find(({ value }) => value === formData?.incident_type)?.name ?? descriptors[0].name}
                        onChange={(e) => {
                            setFormData('incident_type', descriptors.find(({ name }) => name === e.target.value)?.value);
                        }}
                        className="w-full"
                    >
            ...
            <div className="mt-4">
                <div>
                    <label className="block text-sm/6 font-medium text-gray-900">Incident Descriptor</label>
                </div>
                <div className="mt-1 grid grid-cols-1">
                    <SelectInput value={formData.descriptor} onChange={(e) => setFormData('descriptor', e.target.value)} className="w-full">
                        {descriptors.map(
                            ({ options, value }) =>
                                value === formData.incident_type && options.map((option, index) => <option key={index}>{option}</option>),
                        )}
                    </SelectInput>
                </div>
            </div>
            ...
            {formData.descriptor === 'Other' && (
                <div className="mt-4">
                    <div>
                        <label className="block text-sm/6 font-medium text-gray-900">Please briefly Describe:</label>
                    </div>
                    <div className="mt-1">
                        <TextInput
                            required
